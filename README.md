...
3. Place CSV files in replays folder ('./data/replays'). By default, there are two files (126_ESP.csv, 221_ESP.csv) in this folder for demonstration purposes. To process different files, remove these. In addition, in (./data/game_info) there is an overmatch_event_map.json and an overmatch_event_specs.json file. Both of these configure the formatting process and must be changed for different games.
4. Run 'raw_to_stratmapper.py' file in IDE or system command line. Note: the project must be configured this file at this path (note the filepath of any top-level directories).
